...
function BathroomListings(props) {
  ...
  useEffect(() => {
    const getPosts = async () => {
      ...
      const q = query(
        ...
        where("gender", "in", [
          props.male ? "male" : null,
          props.female ? "female" : null,
          props.neutral ? "neutral" : null,
        ]),
  ...
  if (bathroomList.length === 0) {
    return (
      <div className="bathroom-listings-not-found">
  ...
  return (
    <div className="bathroom-listings">
      {bathroomList
        ...
        .map((entry, index) => {
          let current_bathroom_row = (
            <BathroomRow
              className="bathroom-entry"
              key={entry.name}
              id={entry.id}
              name={entry.name}
              image={entry.image}
